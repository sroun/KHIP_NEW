    ...
    <table id="example1" class="table table-bordered table-striped table-hover {{Lang::locale()==='kh' ? 'kh-os' : 'arial'}}">
        ...
        @foreach($jobtype as $c)
            <tr>
                ...
                <td class="center">
                    <a class="cursor-pointer padding-7px"​​ onclick='editJobType("{{$c->id}}","{{$c->pivot->language_id}}")' data-toggle="modal" data-target=".bs-example-modal-sm"><i class="fa fa-edit"></i></a>
                    <a href="#" onclick="deleteJobType('{{$c->id}}')"><i class="fa fa-trash icon-delete"></i></a>
